...
//			var $input = $('<input id="jtable-toolbarsearch-' + fieldName + '" type="text"/>')
			var $input = $('<input id="jtable-toolbarsearch-' + fieldName + '" name="jtable-toolbarsearch-' + field.sqlName + '" type="text"/>')
			...
			$input.bind('change',function(){
				...
				var $i =0;
					$('.jtable-toolbarsearch').each(function(){
						...
						var $field_name = $id.replace('jtable-toolbarsearch-','').toUpperCase();
						var $field_value = $(this).val();
						if($(this).val().length>=1){
							$search_fields = $search_fields + ' AND ' + 
...
//				self.load({'q[]':$q,'opt[]':$opt});
				self.load({'jtFilter':$search_fields});
			});
